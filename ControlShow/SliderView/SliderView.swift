...
class SliderView: UIView {
    ...
    lazy var scaleView: UIView = {
        let scaleView = UIView(frame: .init(x: 0, y: 0, width: 0, height: height))
        scaleView.backgroundColor = UIColor(hexValue: 0xF0F0F0)
        return scaleView
    }()
    
    lazy var valueLab: UILabel = {
        let valueLab = UILabel(frame: .init(x: 15, y: (height - 20) * 0.5, width: 100, height: 20))
        valueLab.font = .italicSystemFont(ofSize: 14)
        valueLab.textColor = .init(hexValue: 0xB3B3B3)
        valueLab.text = "0%"
    ...
    lazy var closeBtn: UIButton = {
        let closeBtn = UIButton(frame: .init(x: width - 15 - 20, y: (height - 20) * 0.5, width: 20, height: 20))
    ...
    lazy var coverView: UIView = {
        let coverView = UIView(frame: .init(x: 0, y: 0, width: width, height: height))
        coverView.backgroundColor = .init(hexValue: 0xFFFFFF, a: 0.2)
    ...
    @objc func panGesture(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .began {
            ...
            scaleView.frame = .init(x: 0, y: 0, width: temp, height: height)
            valueLab.text = "\(Int(temp / width * 100))%"
        } else {
