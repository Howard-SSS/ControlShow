...
class PixelPickerView: UIScrollView {
    ...
    lazy var contentView: UIView = {
        ...
        let contentView = UIView(frame: .init(x: 0, y: (self.height - height) * 0.5, width: width, height: height))
    ...
    func updateViewBaseOn(itemLab: PixelLabel) {
        ...
        for tempY in 0..<y {
            ...
            for tempX in 0..<x {
                ...
                minX += itemLab.width + 2 * offsetX + space
            }
            minX = space
            if rowExistSelect {
    ...
    func reloadItemLabs() {
        ...
        for tempIndex in 0..<(x * y) {
            ...
            lab.backgroundColor = .init(hexValue: 0x262626)
            lab.font = .italicSystemFont(ofSize: 14)
            lab.textColor = .init(hexValue: 0x464646)
